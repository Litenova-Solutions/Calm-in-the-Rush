...
The Nature cover and three Nature gallery tiles are bundled looping video excerpts from Pexels. The cover excerpt is an 8 second slowed portrait loop near 3.8 MB. The three gallery excerpts range near 0.5 MB to 4.6 MB. Each video carries its own bundled ambient bed near 0.5 MB to 0.6 MB: stream sounds, small waves, forest birds and wind, and wind in tall grass. Sound is on by default, starts at the first touch, follows the active view, and never overlaps. The three Quiet Moments photographs are generated for this app. Seven still files remain under `apps/web/public/media/experience`: four legacy Nature photographs in 1024 by 1536 WebP for existing browser-local configurations, and three Quiet Moments photographs. The stored still photographs range from 63 KB to 296 KB. The browser preloads only the opening cover video; later bundled media lazy-load when their active screen renders. Tiles show no on-screen credits because the Pexels license and the CC0 beds need none. The full credit trail lives on the credits page and in the media provenance record. Reduced motion stops every video and shows its poster still. Browser-local uploads use native image elements because a blob URL is not a static bundled asset.

The media provenance record identifies each photographer or prompt, source location, output SHA-256 hash, encoding, and license basis. Video derivatives record their excerpt range, output size, and the muted playback change. The existing supplied RUST SVG logo and its source record remain intact.
